#ifndef CHINRO_LIT_INPUT_INCLUDED
#define CHINRO_LIT_INPUT_INCLUDED
...
InputConfig GetInoutConfig(float2 baseUV, float2 detailUV = 0.0)
{
    InputConfig c;
    c.baseUV = baseUV;
    c.detailUV = detailUV;
...
float3 GetNormalTS (InputConfig c)
{
    ...
    if(c.useDetail)
    {
        ...
        //  下面方法不正确，遮罩没有使用上，但是很奇怪，遮罩在计算detail的时候就已经乘上了
...
float GetOcclusion (InputConfig c)
{
    ...
    {
        occlusion = lerp(0.0, occlusion, strength);
    }
    return occlusion;
}
